...
namespace AILibrary
{
    ...
    public class XmlInputReader : XmlScriptReader
    {
        /// <summary>
        ...
        public XmlInputReader(string filePathOrContent, bool isPath) : base(filePathOrContent, isPath)
        {
        }
        ...
        public int GetMessageCaseNumber(int scriptID)
        {
            if (this.IsNeedThisScript(2, scriptID))
            {
                return int.Parse(base.GetLevelTwoElementValueFromXml("InputNumber", scriptID));
            }

            throw new InvalidOperationException("이 스크립트는 Input 스크립트에 해당하지 않는 스크립트입니다.");
        }
        ...
        public string[] GetMessageCase(int scriptID)
        {
            if (this.IsNeedThisScript(2, scriptID))
            {
                string[] gottenMessage = base.GetLevelTwoElementValueFromXml("InputMessage", scriptID).Split('/');
        ...
        public string[] GetLinkedScript(int scriptID)
        {
            if (this.IsNeedThisScript(2, scriptID))
            {
                string[] gottenMessage = base.GetLevelTwoElementValueFromXml("InputLinkTo", scriptID).Split('/');
                CheckHasWrongLength(gottenMessage, scriptID);
                return gottenMessage;
            }
        ...
        public override Script GetScript(int scriptID)
        {
            if (this.IsNeedThisScript(2, scriptID))
            {
                return new InputScript(this.GetLinkedScript(scriptID), this.GetMessageCase(scriptID));
            }
        ...
        {
            if (toCheck.Length != this.GetMessageCaseNumber(scriptID))
            {
                throw new InvalidOperationException("구해진 메시지의 개수가 설정된 Input 케이스 개수와 다릅니다.");
            }
        }
